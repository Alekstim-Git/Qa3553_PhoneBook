...
#@pytest.mark.skip(reason = "BUG-123: Contact with empty") # пропускает этот тест
@pytest.mark.xfail(reason = "BUG-123: Contact with empty") # пропустит, если тест падает.
...
def test_add_contact_invalid_phone(authenticated_driver):
    ...
    assert contact_page.get_alert_text().strip() == PHONE_ALERT_TEXT  #(strip)- удали все пробелы в начале и конце текста алерта
...
def test_add_contact_invalid_email(authenticated_driver):
    ...
    assert contact_page.get_alert_text().strip() == EMAIL_ALERT_TEXT  # (strip)- удали все пробелы в начале и конце текста алерта
    contact_page.accept_alert()
...
def test_add_contact_duplicate_phone_rejected(authenticated_driver):
    ...
    contacts_page.open_contact_list()
    assert contacts_page.contact_cards_count(shared_phone) == 1 # жду добавление 1 номера тел.
